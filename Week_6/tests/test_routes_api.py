...
def test_get_all_products_success(client, test_app):
    """
    Test GET /products/ returns a list of products when DB has entries.
    """
...
def test_get_all_products_db_error(client):
    """
    Test GET /products/ returns 500 when DB query fails.
    """
    with patch("Week_6.api.routes.Product.query") as mock_query:
...
def test_get_product_found(client, test_app):
    """
    Test GET /products/{id} returns product if it exists.
    """
    with test_app.app_context():
        p = FoodProduct(
...
def test_get_product_not_found(client):
    """
    Test GET /products/{id} returns 404 for missing product.
    """
...
def test_create_product_no_body(client):
    """
    Test POST /products/ returns 400 when request body is empty.
    """
...
def test_create_product_db_error(client):
    ...
    """
    with patch(
        "Week_6.api.routes.db.session.add", side_effect=SQLAlchemyError("DB fail")
    ):
        res = client.post(
            "/products/",
            json={
...
def test_update_product_validation_error(client, test_app):
    """
    Test PUT /products/{id} returns 400 when invalid data is provided.
    """
    with test_app.app_context():
...
def test_update_product_db_error(client, test_app):
    ...
    with patch(
        "Week_6.api.routes.db.session.commit", side_effect=SQLAlchemyError("DB fail")
    ):
...
def test_delete_product_success(client, test_app):
    """
    Test DELETE /products/{id} deletes an existing product.
    """
    with test_app.app_context():
...
def test_delete_product_not_found(client):
    """
    Test DELETE /products/{id} returns 404 for non-existing product.
    """
...
def test_delete_product_db_error(client, test_app):
    ...
    with patch(
        "Week_6.api.routes.db.session.delete", side_effect=SQLAlchemyError("DB fail")
    ):
        res = client.delete(f"/products/{pid}")
        assert res.status_code == 500
