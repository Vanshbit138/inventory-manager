...
def test_product_total_value():
    """
    Test normal product total value calculation.
    """
...
def test_food_product_total_value():
    """
    Test total value for a food product.
    """
    food = FoodProduct(
        product_id=2,
        product_name="Milk",
...
def test_electronic_product_total_value():
    """
    Test total value for an electronic product.
    """
...
def test_book_product_total_value():
    """
    Test total value for a book product.
    """
    book = BookProduct(
        product_id=4,
...
def test_product_zero_price():
    """
    Edge case: Price is zero — should raise ValidationError.
    """
...
def test_product_negative_quantity():
    """
    Edge case: Negative quantity — should raise ValidationError.
    """
    with pytest.raises(ValidationError):
...
def test_product_blank_name():
    """
    Edge case: Blank product name — should raise ValidationError.
    """
    with pytest.raises(ValidationError):
...
def test_electronic_product_zero_warranty():
    """
    Edge case: Warranty period is 0 — should raise ValidationError.
    """
    with pytest.raises(ValidationError):
        ElectronicProduct(
            product_id=8,
...
def test_electronic_product_negative_warranty():
    """
    Edge case: Negative warranty — should raise ValidationError.
    """
    with pytest.raises(ValidationError):
        ElectronicProduct(
            product_id=9,
...
def test_book_product_zero_pages():
    """
    Edge case: Book with 0 pages — should raise ValidationError.
    """
    with pytest.raises(ValidationError):
        BookProduct(
            product_id=11,
...
def test_book_product_blank_author():
    """
    Edge case: Book with blank author — should raise ValidationError.
    """
    with pytest.raises(ValidationError):
        BookProduct(
            product_id=12,
